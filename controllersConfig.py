...
def loadAllControllersConfig() :
    ...
    for controller in root.findall(".//inputConfig") :
        controllerInstance = Controller(controller.get("deviceName"),controller.get("type"), controller.get("deviceGUID"))
...
def loadControllerConfig(p1index, p1guid, p1name, p2index, p2guid, p2name, p3index, p3guid, p3name, p4index, p4guid, p4name) :
    ...
    for controllerGUID in controllers :
        ...
        if controller.guid == p1guid:
            newController = Controller(controller.name, controller.type, controller.guid, p1index, p1name, controller.inputs)
            playerControllers["1"] = newController
        if controller.guid == p2guid:
            newController = Controller(controller.name, controller.type, controller.guid, p2index, p2name, controller.inputs)
            playerControllers["2"] = newController
        if controller.guid == p3guid:
            newController = Controller(controller.name, controller.type, controller.guid, p3index, p3name, controller.inputs)
            playerControllers["3"] = newController
        if controller.guid == p4guid:
            newController = Controller(controller.name, controller.type, controller.guid, p4index, p4name, controller.inputs)
